...
real(8) function v0(x)
implicit none
real(8) x
	v0=50*(exp(-x**2/2.d0)-0.5)
end
...
h=(x2-x1)/dble(n)
x=x1
intf=v0(x)*f(k,x)*f(p,x)
do i=1,n-1
   x=x+h
   intf=intf+dble((mod(i,2)+1)*2)*v0(x)*f(k,x)*f(p,x)
enddo
x=x+h
intf=intf+v0(x)*f(k,x)*f(p,x)
intf=intf*h/3.d0
end function intf
